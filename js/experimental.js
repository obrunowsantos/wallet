...
/* harmony export */   n: () => (/* binding */ mutationWatch)
/* harmony export */ });
/* harmony import */ var _onLoad__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(478);

function mutationWatch(query, process, root = document) {
    (0,_onLoad__WEBPACK_IMPORTED_MODULE_0__/* .onLoad */ .kF)(() => {
...
/* harmony import */ var _functions_watch__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(84);
/* harmony import */ var _functions_onLoad__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(478);
...
(function () {
    ...
    function initializeFromScript() {
        ...
        config.text = currentScript?.getAttribute('data-xtracky-addtocart-by-text');
        config.currentUrl = new URL(window.location.href);
    }
    async function dispatch(data) {
        const endpoint = 'https://api.xtracky.com/api/integrations/addToCart';
        try {
            ...
            console.warn('Erro ao enviar evento de carrinho:', error);
        }
    }
    async function handleAddToCart(event) {
        const target = event.target;
        const utmValue = localStorage.getItem(`KWAI_UTM_TRACK_${config.token}`);
        await dispatch({
            r: btoa(config.currentUrl.href),
            utmSource: utmValue,
        });
        const redirectUrl = target.getAttribute('data-xtracky-redirect');
        if (redirectUrl) {
            window.location.href = redirectUrl + window.location.search;
        }
    }
    function isAddToCart(target) {
        if (!target)
            return;
        const currentScript = state.currentScript;
        if (!currentScript)
            return;
        return isCustomAddToCart(target) || isTypebotAddToCart(target);
    }
    function isCustomAddToCart(target) {
        return target?.matches('[data-xtracky-addtocart]');
    }
    function isTypebotAddToCart(target) {
        if (!target.matches('button'))
            return false;
        if (!config.text)
            return false;
        return target?.innerText === config.text;
    }
    function attachTypebotListener(element) {
        const shadowRootDocument = element.shadowRoot;
        if (!shadowRootDocument)
            return;
        const shadowRootDocumentDoc = shadowRootDocument;
        shadowRootDocumentDoc?.removeEventListener?.('click', listAddtoCart);
        shadowRootDocumentDoc?.addEventListener?.('click', listAddtoCart);
    }
    function initializeAddToCartButtons() {
        document.addEventListener('click', listAddtoCart);
        // Typebot v2 supports multiple embed types: standard, popup, and bubble
        // Watch for all three component types
        const typebotComponents = ['typebot-standard', 'typebot-popup', 'typebot-bubble'];
        typebotComponents.forEach(componentName => {
            (0,_functions_watch__WEBPACK_IMPORTED_MODULE_0__/* .mutationWatch */ .n)(componentName, elements => {
                elements.forEach(attachTypebotListener);
            });
        });
    }
    function listAddtoCart(event) {
        const target = event.target;
        if (!target)
            return;
    ...
    function init() {
        ...
        (0,_functions_onLoad__WEBPACK_IMPORTED_MODULE_1__/* .onLoad */ .kF)(initializeAddToCartButtons);
    }
    // Start the script
    init();
})();
...
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   kF: () => (/* binding */ onLoad)
/* harmony export */ });
/* unused harmony exports isDOMReady, isDocumentLoaded */
/**
...
// EXTERNAL MODULE: ./src/functions/watch.ts
var watch = __webpack_require__(84);
// EXTERNAL MODULE: ./src/functions/onLoad.ts
var onLoad = __webpack_require__(478);
...
function initUTMHandler(hardCodedConfig) {
    ...
    function initCheckoutListeners() {
        ...
        (0,watch/* mutationWatch */.n)('[data-xtracky-checkout]', elements => {
    ...
    function initAdvancedTracking() {
        ...
        selectorRulesMap.forEach((rulesForSelector, selector) => {
            try {
                (0,watch/* mutationWatch */.n)(selector, elements => {
    ...
    async function onMount() {
        initializeFromScript();
        initFingerPrint();
        (0,onLoad/* onLoad */.kF)(handleUtmParameters);
        (0,onLoad/* onLoad */.kF)(initCheckoutListeners);
        (0,onLoad/* onLoad */.kF)(initAdvancedTracking);
        (0,onLoad/* onLoad */.kF)(maybeLoadConfigMode);
        initWatch();
    }
    ...
    function initWatch() {
        ...
        (0,watch/* mutationWatch */.n)('iframe', iframes => iframes.forEach(iframe => {
            if (iframe.src) {
                const store = stores.local.context(getLeadIdStorageKey());
                const leadId = store.get();
                if (leadId) {
...
;// ./src/export/interceptor.ts

initUTMHandler({
    shouldEnableInterception: true
});
...
/* harmony import */ var _interceptor__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(752);
/* harmony import */ var _cart_tracker__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(97);
// Experimental Script v2 - Typebot Integration
// Combines UTM Handler (with interception) + Cart Tracker for Typebot
// Import UTM Handler with navigation interception

// Import Cart Tracker for Typebot shadow DOM support
